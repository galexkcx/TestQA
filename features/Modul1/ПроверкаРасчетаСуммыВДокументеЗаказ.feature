...
* Проверка, что в поле Количество можно ввести значение. Конечно можно, это видно из предыдущей проверки. Ну напишем еще раз. Раз уж требуется.
	...
	Тогда таблица "Товары" содержит строки:
		| 'Товар'		 | 'Цена'     | 'Количество' | 'Сумма'     |
		| '*'	 		 | '2 500,00' | '3'     	 | '7 500,00'  |
* Проверка, что в поле Количество можно ввести значение, если товар является услугой
	Когда открылось окно 'Заказ (создание) *'
	И в таблице "Товары" я нажимаю на кнопку с именем 'ТоварыДобавить'
	И в таблице "Товары" я нажимаю кнопку выбора у реквизита с именем "ТоварыТовар"
	Тогда открылось окно 'Товары'
	...
	И в таблице "Товары" я активизирую поле с именем "ТоварыКоличество"
	И в таблице "Товары" я выбираю текущую строку
* Если возникнет исключение, значит поле Количество не доступно и в результате останется пустым
	Попытка
		И в таблице "Товары" в поле с именем 'ТоварыКоличество' я ввожу текст '3'
	Исключение
		И в таблице "Товары" я завершаю редактирование строки
